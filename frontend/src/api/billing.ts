...
export const cancelSubscription = async (reason?: string) => {
  const response = await axios.post('/api/subscriptions/cancel', { reason });
  return response.data;
};

// Update subscription
export const updateSubscription = async (planId: number) => {
  const response = await axios.post('/api/subscriptions/update', { plan_id: planId });
  return response.data;
};
...
export const addPaymentMethod = async (paymentMethodId: string) => {
  const response = await axios.post('/api/payment_methods', { payment_method_id: paymentMethodId });
  return response.data;
};
...
export const deletePaymentMethod = async (paymentMethodId: string) => {
  const response = await axios.delete(`/api/payment_methods/${paymentMethodId}`);
  return response.data;
};

// Set default payment method
export const setDefaultPaymentMethod = async (paymentMethodId: string) => {
  const response = await axios.post(`/api/payment_methods/${paymentMethodId}/default`);
  return response.data;
};