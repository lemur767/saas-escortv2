import stripe
from flask import current_app
from datetime import datetime, timedelta
from app.models import models

from app.extensions import db
import logging
import json

logger = logging.getLogger(__name__)

def initialize_stripe():
    """Configure Stripe with API key"""
    stripe.api_key = current_app.config['STRIPE_SECRET_KEY']

def create_subscription(user_id, plan_id, payment_method_id):
    """
    Create a new subscription for a user
    """
    initialize_stripe()
    
    # Get user, plan, and payment method
    user = User.query.get(user_id)
    if not user:
        raise ValueError("User not found")
    
    plan = SubscriptionPlan.query.get(plan_id)
    if not plan:
        raise ValueError("Subscription plan not found")
    
    payment_method = PaymentMethod.query.get(payment_method_id)
    if not payment_method:
        raise ValueError("Payment method not found")
    
    # Check payment method belongs to user
    if payment_method.user_id != user_id:
        raise ValueError("Payment method does not belong to user")
    
    # Ensure user has a Stripe customer ID
    if not user.stripe_customer_id:
        customer = stripe.Customer.create(
            email=user.email,
            name=f"{user.first_name} {user.last_name}".strip(),
            phone=user.phone_number
        )
        user.stripe_customer_id = customer.id
        db.session.commit()
    
    # Set the payment method as default for the customer
    stripe.Customer.modify(
        user.stripe_customer_id,
        invoice_settings={
            'default_payment_method': payment_method.stripe_payment_method_id
        }
    )
    
    # Create Stripe subscription
    stripe_subscription = stripe.Subscription.create(
        customer=user.stripe_customer_id,
        items=[
            {'price': plan.stripe_price_id},
        ],
        payment_behavior='default_incomplete',
        expand=['latest_invoice.payment_intent'],
        metadata={
            'user_id': user_id,
            'plan_id': plan_id
        }
    )
    
    # Determine renewal/end dates based on billing cycle
    start_date = datetime.fromtimestamp(stripe_subscription.current_period_start)
    if plan.billing_cycle == 'monthly':
        renewal_date = start_date + timedelta(days=30)
    elif plan.billing_cycle == 'yearly':
        renewal_date = start_date + timedelta(days=365)
    else:
        renewal_date = start_date + timedelta(days=30)  # Default to monthly
    
    meta = {
        'created_by': 'billing_service',
        'version': '1.0',
        'initial_plan': plan.name
    }
    
    # Create subscription record in database
    subscription = Subscription(
        user_id=user_id,
        plan_id=plan_id,
        stripe_subscription_id=stripe_subscription.id,
        stripe_customer_id=user.stripe_customer_id,
        payment_method_id=payment_method_id,
        start_date=start_date,
        renewal_date=renewal_date,
        is_active=True,
        payment_status=stripe_subscription.status,
        auto_renew=True,
        subscription_metadata=json.dumps(meta)  # Use the new column name
    )
    
    db.session.add(subscription)
    db.session.commit()
    
    from app.models.subscription_payment_method import SubscriptionPaymentMethod
    spm = SubscriptionPaymentMethod(
        subscription_id=subscription.id,
        payment_method_id=payment_method_id,
        is_primary=True
    )
    db.session.add(spm)
    db.session.commit()
    
    # Create invoice record if present
    if hasattr(stripe_subscription, 'latest_invoice') and stripe_subscription.latest_invoice:
        invoice = Invoice(
            user_id=user_id,
            subscription_id=subscription.id,
            payment_method_id=payment_method_id,
            stripe_invoice_id=stripe_subscription.latest_invoice.id,
            amount=stripe_subscription.latest_invoice.amount_due / 100,  # Convert cents to dollars
            currency=stripe_subscription.latest_invoice.currency.upper(),
            status=stripe_subscription.latest_invoice.status,
            invoice_date=datetime.fromtimestamp(stripe_subscription.latest_invoice.created),
            invoice_pdf_url=stripe_subscription.latest_invoice.invoice_pdf
        )
        
        db.session.add(invoice)
        db.session.commit()
    
    return subscription

def update_subscription(subscription_id, plan_id):
    """
    Update a subscription to a new plan
    """
    initialize_stripe()
    
    # Get subscription and new plan
    subscription = Subscription.query.get(subscription_id)
    if not subscription:
        raise ValueError("Subscription not found")
    
    new_plan = SubscriptionPlan.query.get(plan_id)
    if not new_plan:
        raise ValueError("Subscription plan not found")
    
    # Check for actual plan change
    if subscription.plan_id == plan_id:
        return subscription  # No change needed
    
    try:
        # Update Stripe subscription
        stripe_subscription = stripe.Subscription.retrieve(subscription.stripe_subscription_id)
        
        # Get the subscription item ID (assuming one item per subscription)
        item_id = stripe_subscription['items']['data'][0].id
        
        # Update the subscription item with the new price
        stripe.Subscription.modify(
            subscription.stripe_subscription_id,
            items=[{
                'id': item_id,
                'price': new_plan.stripe_price_id,
            }],
            proration_behavior='create_prorations',
            metadata={
                'user_id': subscription.user_id,
                'plan_id': plan_id
            }
        )
        
        # Update local subscription
        subscription.plan_id = plan_id
        
        # Update renewal date based on new billing cycle
        if new_plan.billing_cycle == 'monthly':
            subscription.renewal_date = subscription.start_date + timedelta(days=30)
        elif new_plan.billing_cycle == 'yearly':
            subscription.renewal_date = subscription.start_date + timedelta(days=365)
        
        db.session.commit()
        
        return subscription
    
    except stripe.error.StripeError as e:
        logger.error(f"Stripe error updating subscription: {str(e)}")
        raise ValueError(f"Error updating subscription: {str(e)}")

def cancel_subscription(subscription_id):
    """
    Cancel a subscription
    """
    initialize_stripe()
    
    # Get subscription
    subscription = Subscription.query.get(subscription_id)
    if not subscription:
        raise ValueError("Subscription not found")
    
    try:
        # Cancel at period end to avoid immediate cancellation and proration issues
        stripe.Subscription.modify(
            subscription.stripe_subscription_id,
            cancel_at_period_end=True
        )
        
        # Update local subscription
        subscription.auto_renew = False
        subscription.end_date = subscription.get_current_period_end()
        
        db.session.commit()
        
        return subscription
    
    except stripe.error.StripeError as e:
        logger.error(f"Stripe error canceling subscription: {str(e)}")
        raise ValueError(f"Error canceling subscription: {str(e)}")

def check_subscription_status(subscription_id):
    """
    Check current status of a subscription with Stripe
    """
    initialize_stripe()
    
    # Get subscription
    subscription = Subscription.query.get(subscription_id)
    if not subscription:
        raise ValueError("Subscription not found")
    
    try:
        # Get Stripe subscription
        stripe_subscription = stripe.Subscription.retrieve(subscription.stripe_subscription_id)
        
        # Update local status
        subscription.payment_status = stripe_subscription.status
        
        if stripe_subscription.status in ['canceled', 'unpaid']:
            subscription.is_active = False
            subscription.end_date = datetime.fromtimestamp(stripe_subscription.ended_at) if hasattr(stripe_subscription, 'ended_at') and stripe_subscription.ended_at else datetime.utcnow()
        
        db.session.commit()
        
        return subscription
    
    except stripe.error.StripeError as e:
        logger.error(f"Stripe error checking subscription: {str(e)}")
        raise ValueError(f"Error checking subscription: {str(e)}")

def create_usage_record(subscription_id, quantity, timestamp=None, action='increment'):
    """
    Record usage for a metered subscription
    """
    initialize_stripe()
    
    # Get subscription
    subscription = Subscription.query.get(subscription_id)
    if not subscription:
        raise ValueError("Subscription not found")
    
    try:
        # Get Stripe subscription
        stripe_subscription = stripe.Subscription.retrieve(subscription.stripe_subscription_id)
        
        # Get subscription item (assuming one item per subscription)
        item_id = stripe_subscription['items']['data'][0].id
        
        # Create usage record
        usage_record = stripe.SubscriptionItem.create_usage_record(
            item_id,
            quantity=quantity,
            timestamp=int(timestamp.timestamp()) if timestamp else int(datetime.utcnow().timestamp()),
            action=action  # 'increment' or 'set'
        )
        
        return usage_record
    
    except stripe.error.StripeError as e:
        logger.error(f"Stripe error recording usage: {str(e)}")
        raise ValueError(f"Error recording usage: {str(e)}")

def create_customer_portal_session(user_id, return_url):
    """
    Create a Stripe customer portal session
    """
    initialize_stripe()
    
    # Get user
    user = User.query.get(user_id)
    if not user:
        raise ValueError("User not found")
    
    if not user.stripe_customer_id:
        raise ValueError("User does not have a Stripe customer ID")
    
    try:
        # Create portal session
        session = stripe.billing_portal.Session.create(
            customer=user.stripe_customer_id,
            return_url=return_url
        )
        
        return session.url
    
    except stripe.error.StripeError as e:
        logger.error(f"Stripe error creating portal session: {str(e)}")
        raise ValueError(f"Error creating portal session: {str(e)}")

def create_checkout_session(user_id, plan_id, success_url, cancel_url):
    """
    Create a Stripe checkout session for a subscription
    """
    initialize_stripe()
    
    # Get user and plan
    user = User.query.get(user_id)
    if not user:
        raise ValueError("User not found")
    
    plan = SubscriptionPlan.query.get(plan_id)
    if not plan:
        raise ValueError("Subscription plan not found")
    
    # Create or retrieve Stripe customer
    if not user.stripe_customer_id:
        customer = stripe.Customer.create(
            email=user.email,
            name=f"{user.first_name} {user.last_name}".strip(),
            phone=user.phone_number
        )
        user.stripe_customer_id = customer.id
        db.session.commit()
    
    try:
        # Create checkout session
        session = stripe.checkout.Session.create(
            customer=user.stripe_customer_id,
            payment_method_types=['card'],
            line_items=[{
                'price': plan.stripe_price_id,
                'quantity': 1,
            }],
            mode='subscription',
            success_url=success_url,
            cancel_url=cancel_url,
            metadata={
                'user_id': user_id,
                'plan_id': plan_id
            }
        )
        
        return session.id, session.url
    
    except stripe.error.StripeError as e:
        logger.error(f"Stripe error creating checkout session: {str(e)}")
        raise ValueError(f"Error creating checkout session: {str(e)}")

def seed_default_plans():
    """
    Create default subscription plans in the database and Stripe
    """
    initialize_stripe()
    
    # Define default plans
    default_plans = [
        {
            'name': 'Free Trial',
            'description': 'Basic features with limited usage',
            'price': 0.00,
            'billing_cycle': 'monthly',
            'max_profiles': 1,
            'monthly_message_limit': 100,
            'monthly_ai_response_limit': 50,
            'message_history_days': 7,
            'features': {
                'ai_responses': True,
                'auto_replies': True,
                'business_hours': True,
                'analytics': False,
                'advanced_ai': False
            },
            'is_active': True,
            'is_featured': False,
            'sort_order': 1
        },
        {
            'name': 'Basic',
            'description': 'Essential features for individuals',
            'price': 19.99,
            'billing_cycle': 'monthly',
            'max_profiles': 1,
            'monthly_message_limit': 1000,
            'monthly_ai_response_limit': 500,
            'message_history_days': 30,
            'features': {
                'ai_responses': True,
                'auto_replies': True,
                'business_hours': True,
                'analytics': True,
                'advanced_ai': False
            },
            'is_active': True,
            'is_featured': False,
            'sort_order': 2
        },
        {
            'name': 'Professional',
            'description': 'Advanced features for power users',
            'price': 49.99,
            'billing_cycle': 'monthly',
            'max_profiles': 3,
            'monthly_message_limit': 5000,
            'monthly_ai_response_limit': 2500,
            'message_history_days': 90,
            'features': {
                'ai_responses': True,
                'auto_replies': True,
                'business_hours': True,
                'analytics': True,
                'advanced_ai': True,
                'priority_support': True
            },
            'is_active': True,
            'is_featured': True,
            'sort_order': 3
        },
        {
            'name': 'Enterprise',
            'description': 'Unlimited features for businesses',
            'price': 99.99,
            'billing_cycle': 'monthly',
            'max_profiles': 10,
            'monthly_message_limit': None,  # Unlimited
            'monthly_ai_response_limit': None,  # Unlimited
            'message_history_days': 365,
            'features': {
                'ai_responses': True,
                'auto_replies': True,
                'business_hours': True,
                'analytics': True,
                'advanced_ai': True,
                'priority_support': True,
                'white_label': True,
                'api_access': True
            },
            'is_active': True,
            'is_featured': False,
            'sort_order': 4
        }
    ]
    
    for plan_data in default_plans:
        # Check if plan already exists in database
        existing_plan = SubscriptionPlan.query.filter_by(name=plan_data['name']).first()
        
        if not existing_plan:
            # Create product in Stripe (skip for free plan)
            if plan_data['price'] > 0:
                # Create product in Stripe
                product = stripe.Product.create(
                    name=plan_data['name'],
                    description=plan_data['description'],
                    metadata={
                        'plan_type': plan_data['name'].lower().replace(' ', '_')
                    }
                )
                
                # Create price in Stripe
                price = stripe.Price.create(
                    product=product.id,
                    unit_amount=int(plan_data['price'] * 100),  # Convert to cents
                    currency='usd',
                    recurring={
                        'interval': 'month' if plan_data['billing_cycle'] == 'monthly' else 'year'
                    }
                )
                
                # Add Stripe IDs to plan data
                plan_data['stripe_product_id'] = product.id
                plan_data['stripe_price_id'] = price.id
            
            # Create features JSON
            features = plan_data.pop('features')
            
            # Create plan in database
            plan = SubscriptionPlan(**plan_data)
            plan.set_features(features)
            
            db.session.add(plan)
            db.session.commit()
            
            logger.info(f"Created subscription plan: {plan.name}")
        else:
            logger.info(f"Subscription plan already exists: {existing_plan.name}")
    
    logger.info("Default plans seeded successfully")